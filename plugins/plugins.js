...
const REPO_OWNER = 'PrinceXtremeX';
const REPO_NAME = 'BOYKA-XD';
...
async (conn, mek, m, { from, reply }) => {
    try {
        ...
        let pluginList = "📂 *Boyka xd Plugins:*\n\n";
...
async (conn, mek, m, { from, reply, args, senderNumber }) => {
    try {
        ...
        if (m.quoted && m.quoted.key.fromMe) {
            ...
            if (/📂 \*xtreme xmd Plugins:\*/i.test(quotedText)) {
        ...
        const statusMessage = {
            image: { url: `https://i.ibb.co/tp3jnw3s/jpg.jpg` }, // Replace with your image URL
